...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idmateria)){
			...
			echo $rspta ? "Genero registrado" : "Genero no se pudo registrar";
		}
		else {
			$rspta=$materia->editar($idmateria,$nombre,$descripcion);
			echo $rspta ? "Genero actualizado" : "Genero no se pudo actualizar";
		}
	break;

	case 'desactivar':
		$rspta=$materia->desactivar($idmateria);
 		echo $rspta ? "Genero Desactivado" : "Genero no se puede desactivar";
	break;

	case 'activar':
		$rspta=$materia->activar($idmateria);
 		echo $rspta ? "Genero activado" : "Genero no se puede activar";
	break;
